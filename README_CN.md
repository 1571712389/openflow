...
### 🛡️ 质量治理，而不是最后补救
OpenFlow 对代码质量的控制分成两层：
- **前置约束**：默认开启 TDD 计划增强，把测试与验证要求前置到实现计划中
- **后置加固**：对复杂改动可启用 `harden`，通过 reviewer / executor 多轮对抗，反复检查实现是否偏离设计、是否存在回归风险、是否缺少验证
- **最终门禁**：进入 `verify` 后，仍然要用 evidence 和 readiness 判断这次改动是否真的具备完成条件
