...
[头脑风暴 →] feature → writing-plan → implement → quality-gate → archive
      ↑  可选                       ↑
  纯对话探索               两条实现路径:
  需求不清时先用            1. OpenCode: /openflow-implement → build
  再进入 feature            2. omo: /openflow-implement → /start-work
```
...
/openflow-implement <功能名>           # 启动可追踪的实现运行
...
#### 4. 实现阶段：`/openflow-implement <功能名>`
设计和计划准备好后，用这个命令启动一轮可追踪的实现运行：
- **作用**：创建 `ImplementationRun`，可选创建隔离 git worktree，绑定观察者事件，并把执行交给可用后端。
- **配合 omo**：检测到 omo 后交接到 `/start-work <功能名>`。
- **不使用 omo**：交接到 OpenCode 原生 build 流程。
